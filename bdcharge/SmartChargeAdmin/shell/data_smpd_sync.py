...
def deal_match():
    ...
    #日志文件初始化
    log, fh = loger_init()
    log.info('扫码配对工装登记扫码信息同步到sqlserver数据库!')
    ...
    try:
        ...
        log.info("SqlServer数据库连接失败!"+str(e) )
        return  -1
    ...
    try:
        ...
        for item in rows:
            ...
            log.info("due num:" + str(i))
    except Exception as ex:
        log.info("MySql数据库连接失败!"+str(ex) )

    finally:
        #关闭链接
        mysql_conn.close()
        sqlserver_conn.close()
        log.info("扫码配对工装登记扫码信息同步到sqlserver数据库完成! 此次处理记录:" + str(i) )
