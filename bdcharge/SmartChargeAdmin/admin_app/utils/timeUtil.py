...
# -*- coding: UTF-8 -*-
"""=================================================
@Project -> File   ：WaterInvoice -> timeutil.py
@IDE    ：PyCharm
@Author ：Mr. Dzy
@Date   ：2023/5/29 17:55
@Description  ：日期、时间处理工具类
@Copyright：©2023-2023 河南品码信息科技有限公司
=================================================="""
...
def generate_10timestamp():
    """根据当前时间获取时间戳，返回整数(十位)"""
    return int(time.time())


def generate_13timestamp():
    """
    毫秒级时间戳(十三位)
    """
...
def get_current_time_formatted(time_format="%y%m%d%H%M%S"):
...
def time_is_recent(dt, **kwargs):
    ...
    if dt and isinstance(dt, datetime.datetime) and kwargs and (
            not set(kwargs.keys()) - {'days', 'hours', 'minutes', 'seconds'}):
...
def process_str_to_datetime(date_input, input_format='%Y-%m-%d %H:%M:%S', output_format='%Y-%m-%d %H:%M:%S'):
    if isinstance(date_input, str):
        # 将字符串转换为datetime对象
        date_object = datetime.datetime.strptime(date_input, input_format)
        # 将datetime对象转换为指定格式的字符串
        formatted_date_string = date_object.strftime(output_format)
    else:
        formatted_date_string = date_input.strftime(output_format)
...
def generate_random_datetime():
    ...
    # 创建一个包含最近七天日期的列表
    # 注意：这里我们仅保留日期部分，去掉时间部分
    dates = [now - datetime.timedelta(days=x) for x in range(7)]
    dates = [d.date() for d in dates]  # 转换为date对象，去掉时间部分

    # 随机选择一个日期
    random_date = random.choice(dates)

    # 如果你想要一个完整的datetime对象（包括时间部分），并且想要随机时间，可以这样做：
    # 注意：这里的时间部分是随机的，从00:00:00到23:59:59
    random_hour = random.randint(0, 23)
    random_minute = random.randint(0, 59)
    random_second = random.randint(0, 59)
    random_time = datetime.time(random_hour, random_minute, random_second)
    # 将随机日期和随机时间组合成一个datetime对象
