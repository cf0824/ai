...
# 用户使用密码登陆时，获取md5随机数
def user_login_by_getsalt(request):
    log = public.logger
    body = public.req_body
    login_name = body.get('login_name', '0')
    try:
        cur = connection.cursor()  # 创建游标
        sql = "select user_id, salt, state from sys_user where user_id=%s or login_name=%s "
        cur.execute(sql, (login_name, login_name))
        row = cur.fetchone()
        if not row:
            cur.close()
...
def get_user_info(request):
    ...
    try:
        ...
        sql = "select user_name, login_name, passwd, station, certi_type, certi, sex, address, tel, email, state, head_imgurl, snote, now_org_id " \
    ...
    # 如果有now_org_id，查询机构名称
    db_now_org_name = ''
    if db_now_org_id:
        sql = "select org_name from sys_org where org_id=%s"
        cur.execute(sql, db_now_org_id)
        row = cur.fetchone()
        if row and row[0]:
            db_now_org_name = row[0]
    ...
    json_data = {
        ...
        "BODY": {
            ...
            "head_imgurl": db_head_imgurl,
            "snote": db_snote,
            "now_org_id": str(db_now_org_id),
            "now_org_name": db_now_org_name,
            "user_form_var": {
...
def user_login_out(request):
    ...
    public.respcode, public.respmsg = "000000", "用户退出登陆成功!"
    json_data = {
        "HEAD": public.resphead_setvalue(),
        "BODY": {
            "uid": public.user_id
...
def add_user_info(request):
    ...
    try:
        ...
        sql = "insert into sys_user(user_name,login_name,salt, passwd,station,certi_type, certi,sex,address,tel, " \
              "email, state, head_imgurl,operate_userid, snote) " \
              "values(%s,%s,%s,%s,%s, %s, %s,%s,%s,%s,%s, %s,%s,%s,%s)"
        insert_paras = (user_name, login_name, salt, passwd, station, certi_type, certi, sex, address, tel,
                        email, state, head_imgurl, public.user_id, snote)
...
def update_user_info(request):
    ...
    try:
        # 查找管理台用户信息表权限
        cur = connection.cursor()
        sql = "select user_name, login_name, salt, passwd, station, certi_type, certi, sex, address, tel, " \
              "email, state, head_imgurl, snote from sys_user where user_id='%s'" % public.user_id
        ...
        salt = form_var.get('salt', db_salt)

        sql = "update sys_user set login_name=%s, user_name=%s, salt=%s, passwd=%s, station=%s,certi_type=%s, " \
              "certi=%s, sex=%s, address=%s, tel=%s,email=%s, state=%s, head_imgurl=%s, operate_userid=%s, " \
              "operate_datetime=%s, snote=%s where user_id=%s"
        log.info("用户信息更新:" + sql % (login_name, user_name, salt, passwd_new, station, certi_type,
                                          certi, sex, address, tel, email, state, head_imgurl, public.user_id,
                                          datetime.datetime.now(), snote, userid), extra={'ptlsh': public.req_seq})
        cur.execute(sql, (login_name, user_name, salt, passwd_new, station, certi_type, certi, sex, address, tel,
                          email, state, head_imgurl, public.user_id, datetime.datetime.now(), snote, userid))

        # 用户角色处理
        userrole = form_var.get('user_role')
        # 只删除拥有权限的角色 20221209修改
        if public_db.is_super_admin(public.user_id):
            sql = "delete from sys_user_role where user_id=%s"
            cur.execute(sql, userid)
        else:
            sql = "delete from sys_user_role where user_id=%s and role_id in (select role_id from (select role_id from sys_user_role where user_id =%s) tmp)"
            cur.execute(sql, (userid, public.user_id))
        for item in userrole:
            sql = "insert into sys_user_role(user_id, role_id, user_above_id, operate_userid) values(%s, %s, %s, %s)"
            cur.execute(sql, (userid, item, public.user_id, public.user_id))
...
# 用户自己修改用户信息
def update_user_info_myself(request):
    log = public.logger
    body = public.req_body
    form_var = body.get('form_var')
    cur = connection.cursor()
    try:
        ...
        user_name = form_var.get('user_name')
        user_sex = form_var.get('user_sex')
        user_tel = form_var.get('user_tel')
        head_imgurl = str(form_var.get('head_imgurl', ''))
        now_org_id = form_var.get('now_org_id')
        sql = "update sys_user set user_name=%s, sex=%s, tel=%s, head_imgurl=%s where user_id=%s"
        log.info("用户信息更新:" + sql % (user_name, user_sex, user_tel, head_imgurl, public.user_id),
                 extra={'ptlsh': public.req_seq})
        cur.execute(sql, (user_name, user_sex, user_tel, head_imgurl, public.user_id))
...
def change_now_org(request):
    ...
    try:
        now_org_id = form_var.get('now_org_id')
        sql = "update sys_user set now_org_id=%s where user_id=%s"
        log.info("用户信息更新:" + sql % (now_org_id, public.user_id),
                 extra={'ptlsh': public.req_seq})
        cur.execute(sql, (now_org_id, public.user_id))
    ...
    public.respcode, public.respmsg = "000000", "机构切换成功!"
    json_data = {
        "HEAD": public.resphead_setvalue(),
        "BODY": {
            "form_var": form_var,
    ...
    if str(form_id) == '10102':
        for key, value in kvs.items():
            if key == 'user_id':
                ...
                sql = "select dict_target from sys_ywty_dict where dict_name='CAR_TYPE' and dict_code='%s'" % value
                cur.execute(sql)
                kvs[key] = cur.fetchone()[0]
            elif key == 'status':
...
def getFormTable(cur, form_id, gl_id):
    ...
    if selsql:  # 直接配置查询的SQL语句-支持联表查询
        ...
        # 获取记录字段名列表
        sql = "select field_id,field_name from sys_form_cfg_fieldlist where form_id='%s' and comp_type in ('textarea', 'input', 'datetime', 'select', 'radio', 'date')" % (
            form_id)
        cur.execute(sql)
        rows = cur.fetchall()
        tableHead = []
...
def workflow_apply_handle(request):
    ...
    try:
        ...
        if result:
            ...
            node_state = '2'
        # 将流程节点状态更新
        sql = "update sys_workflow_node_list set node_state='%s',notes='%s' where node_id='%s'" % (
        node_state, remark, node_id)
        cur.execute(sql)
        ...
        if flag != 0:
            # 根据node_id查询wf_id
            sql = "select wf_id from sys_workflow_node_list where node_id='%s'" % node_id
            cur.execute(sql)
            wf_id = cur.fetchone()[0]
            # 根据wf_id查询form_id
            sql = "select form_id from sys_workflow_node_cfg where wf_id='%s'" % wf_id
            ...
            if flag == 1:  # 审核通过执行操作
                ...
                cur.execute(sql)
                # 将list表里的发起状态改为1
                sql = "update sys_workflow_node_list set node_state='1',update_date=%s where order_number=%s and wf_type='apply_user'"
                ...
                cur.execute(sql)
                # 将list表里的发起状态改为2
                sql = "update sys_workflow_node_list set node_state='2',update_date=%s where order_number=%s and wf_type='apply_user'"
                cur.execute(sql, (datetime.datetime.now(), order_number))
...
def get_wait_apply_list(request):
    ...
    try:
        ...
        # 待审批列表 当前到达该用户节点的才返回（考虑前边审核失败的情况）
        # 查询待审批列表
        sql = "select node_id,gl_id,node_state,update_date,order_number from (select  A.node_id,A.gl_id,A.node_state,A.update_date,A.order_number from sys_workflow_node_list A where  A.user_id='%s' and A.node_prev=0 and A.node_state='0' and A.wf_type!='apply_user' UNION select A.node_id,A.gl_id,A.node_state,A.update_date,A.order_number from sys_workflow_node_list A inner join sys_workflow_node_list B on A.node_prev=B.node_id where A.user_id='%s' and B.node_state='1' and A.node_state='0' and A.wf_type!='apply_user') as T where 1=1 %s limit %s,%s" % (
        public.user_id, public.user_id, otherWhere, pageSize * (currentPage - 1), pageSize)
        log.info('sql=%s' % sql)
        cur.execute(sql)
        rows = cur.fetchall()
        print(rows)
...
def get_my_apply_list(request):
    ...
    try:
        ...
        sql = "select a.node_id,a.order_number,a.user_id,a.node_state,a.insert_date,b.wf_name from " \
              "(select node_id,order_number,user_id,node_state,insert_date,form_id from sys_workflow_node_list where wf_type='apply_user' " \
              "union all " \
              "select node_id,order_number,user_id,node_state,insert_date,form_id from sys_workflow_node_list_his where wf_type='apply_user') " \
              "as a left join sys_workflow_tran b on a.form_id=b.form_id where user_id=%s %s limit %s,%s" % \
              (public.user_id, otherWhere, pageSize * (currentPage - 1), pageSize)
        cur.execute(sql)
        rows = cur.fetchall()
...
def user_delete(request):
    ...
    if not user_id:
        public.respcode, public.respmsg = "900001", "删除用户不能为空!"
        public.respinfo = HttpResponse(public.setrespinfo())
        return public.respinfo
    ...
    public.respcode, public.respmsg = "000000", "删除成功!"
    json_data = {
        "HEAD": public.resphead_setvalue(),
        "BODY": {},
    }
...
# 获取机构级联数据 20221208新增
def get_org_cascader_data(request):
    data = [{
        'label': '橙色',
        'value': 'cs'
    }, {
    ...
    public.respcode, public.respmsg = "000000", "success!"
    json_data = {
        "HEAD": public.resphead_setvalue(),
        "BODY": {
            'data': data
...
def add_user_group(request):
    ...
    try:
        # 查找管理台用户信息表权限
        cur = connection.cursor()
        sql = "select user_name from sys_user where user_name='%s'" % user_name
        cur.execute(sql)
        row = cur.fetchone()
        if not row:
            cur.close()
            public.respcode, public.respmsg = "101011", "用户不存在!"
            public.respinfo = HttpResponse(public.setrespinfo())
            return public.respinfo

        # 用户分组处理
        group_list = form_var.get('group_list')
        sql = "delete from auth_user_groups where user_id=%s "
        cur.execute(sql, userid)
        for item in group_list:
            sql = "insert into auth_user_groups(user_id, group_id) values(%s, %s)"
            cur.execute(sql, (userid, item))

        cur.close()

    except Exception as ex:
        log.error("用户分组信息添加失败!" + str(ex), exc_info=True, extra={'ptlsh': public.req_seq})
        public.exc_type, public.exc_value, public.exc_traceback = sys.exc_info()
        cur.close()
        public.respcode, public.respmsg = "101010", "用户分组信息添加失败!" + str(ex)
        public.respinfo = HttpResponse(public.setrespinfo())
        return public.respinfo

    public.respcode, public.respmsg = "000000", "用户分组信息添加成功!"
    json_data = {
        "HEAD": public.resphead_setvalue(),
        "BODY": {
            "user_name": user_name,
