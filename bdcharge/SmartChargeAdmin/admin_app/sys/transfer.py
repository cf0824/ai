...
def transfer_userrole( request ):
    ...
    try:
        ...
        # 登录用户有权限的角色
        # 超级管理员拥有所有角色权限
        is_super_admin = public_db.is_super_admin(public.user_id)
        if is_super_admin:
            sql = "select role_id,role_name from sys_role where role_state='1'"
            cur.execute(sql)
        else:
            sql = "select role_id,role_name from sys_role where role_state='1' and role_id in (select role_id from sys_user_role where user_id=%s) "
        ...
        if form_data.get('user_id'):
            ...
            rows=cur.fetchall()
            for item in rows:
                if item[0] not in transfer_keys:
                    continue
...
def get_group_list(request):
    ...
    try:
        ...
        if is_super_admin:
            ...
            sql = "select id,name from auth_group  where id in (select group_id from auth_user_groups where user_id=%s) "
            cur.execute(sql, public.user_id)
        rows = cur.fetchall()
