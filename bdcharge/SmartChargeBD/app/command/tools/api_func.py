...
class ApiFunc:
    ...
    def Link_interface_resp_login(self, recv_data, special_data):
        ...
        try:
            ...
            return resp_data, byte_resp_data
        ...
        try:
            ...
            if special_data is None:
                ...
                Specific_data_json = {
                    ...
                    'order_no': '00000001',  # 订单号,4字节
                    'balance': '00000001'  # 卡余额,4字节
                }
            else:
                Specific_data_json = special_data
    ...
    def Set_parameters_req_Commu_para(self, json):
        try:
            ...
            #
            # 数据单元
            heart_cycle = Specific_data_json.get('heart_cycle')
            uplink_interval = Specific_data_json.get('up_cycle')
            delay_time = Specific_data_json.get('delay_time')
            heart_cycle = hex(heart_cycle).replace('0x', '').zfill(4).upper()
            uplink_interval = hex(uplink_interval).replace('0x', '').zfill(4).upper()
            delay_time = hex(delay_time).replace('0x', '').zfill(4).upper()

            Specific_data_json = {
                'heart_cycle': heart_cycle,  # 心跳周期
                'uplink_interval': uplink_interval, # 上送间隔
                'delay_time': uplink_interval,
            }
            Specific_data = ''.join(Specific_data_json.values())
            user_data_area = app_head + Specific_data
    ...
    def Set_parameters_req_Domain_port(self, json):
        try:
            ...
            # 2)长度
            length_ = Specific_data_json.get('length')
            domain_len = hex(length_).replace('0x', '').zfill(2).upper()
            # 3)端口
    ...
    def Set_parameters_req_QRcode_down(self, json):
        try:
            ...
            QR_data = Specific_data_json.get('QR_data')
            QR_data_hex = self.apitool.string_to_hex(QR_data)

            Specific_data_json = {
                'QR_len': QR_len,  # 二维码长度
                'QR_data': QR_data_hex  # 二维码内容
            }
            Specific_data = ''.join(Specific_data_json.values())
            user_data_area = app_head + Specific_data
            # 三步：计算长度、计算crc、组合所有片段
    ...
    def get_active_send_data(self, json_data):
        ...
        Function_mapping = {
            '04': {    # 设置参数
                ...
                '17': self.Set_parameters_req_Power_threshold,
                '20': self.Set_parameters_req_Socket_start_stop,
                '21': self.Set_parameters_req_QRcode_down
            },
            '0A': {    # 查询参数
        ...
        resp_data, byte_resp_data = func(json_data)
        # self.log.info(f'type:{type(resp_data)},{type(byte_resp_data)}')
        return resp_data, byte_resp_data
