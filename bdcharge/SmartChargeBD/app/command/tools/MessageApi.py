...
class Message:
    ...
    def get_first_message(self, recv_data):
        ...
        hex_data = recv_data
        if isinstance(recv_data, bytes): # 如果是字节类型，就转成十六进制字符串
        ...
        if hex_data[0:2] != self.head:  # 接收的数据头 不正确，舍弃
            self.log.error(f'接收的数据头 不正确，舍弃! recv_data={recv_data}')
        ...
        if hex_data[str_length-2:str_length] != self.tail:  # 尾部不正确
            self.log.error(f'报文尾部错误! recv_data={recv_data}')
    ...
    def get_func(self, AFN, Fn):
        ...
        try:
            ...
            self.log.error(f"接口匹配失败，找不到接口！AFN={AFN},Fn={Fn}", exc_info=True)
            return None

        if func:
            return func
        else:
            self.log.error(f"接口匹配失败，找不到接口！not func! AFN={AFN},Fn={Fn}")
            return None
        ...
        try:
            ...
            dict_data = {
                ...
                'app_region': {
                    ...
                    'app_region_SEQ': {
                        'app_region_SEQ_': app_region_SEQ,
    ...
    def message_pack1(self, recv_data, special_data):
        ...
        try:
            dict_data = recv_data
            if not isinstance(recv_data, dict):  # 如果不是字典类型，说明没有解包，进行解包
            ...
            resp_data, byte_resp_data = func(dict_data, special_data)
            # self.log.info(f'接口{func}返回的结果:{resp_data},{byte_resp_data}')
            # self.log.info(f'**************************pack1 end*****************************')
            return resp_data, byte_resp_data

        except Exception as e:
            # self.log.info(f'**************************pack1 error***************************')
            self.log.error("pack1 error. %s" % str(e), exc_info=True)
            return False
